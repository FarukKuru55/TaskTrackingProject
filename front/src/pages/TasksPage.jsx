...
function TasksPage() {
  ...
  // 🚀 YENİ: Veritabanından gelecek olan listeler için tepsilerimiz (state)
  const [priorities, setPriorities] = useState([]);
  const [statuses, setStatuses] = useState([]);

  // Sayfa ilk açıldığında hem görevleri hem de seçenekleri çekiyoruz
  useEffect(() => { 
    fetchTasks(); 
    fetchOptions(); // 🚀 Yeni fonksiyonumuzu burada çağırıyoruz
  }, []);

  // 🚀 YENİ: Veritabanındaki SQL ile girdiğimiz verileri çeken fonksiyon
  const fetchOptions = () => {
    api.get('/Priorities/getall').then(res => setPriorities(res.data.data || []));
    api.get('/TaskStatuses/getall').then(res => setStatuses(res.data.data || []));
  };
  ...
  const handleDeleteTask = (taskId) => {
    Swal.fire({
      title: 'Emin misiniz?', text: "Bu görev silinecek!", icon: 'warning',
  ...
  return (
    <div className="p-4">
        ...
        <div className="card-body py-3">
          <div className="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
             <div className="d-flex align-items-center">
                ...
                <span className="badge bg-primary-subtle text-primary border border-primary-subtle">{filteredTasks.length} Görev</span>
             </div>
      ...
      {/* 🚀 MODAL'LARIMIZA VERİLERİ GÖNDERİYORUZ */}
      <TaskAddModal 
        show={showModal} 
        handleClose={() => setShowModal(false)} 
        handleSave={handleSaveTask} 
        selectedTask={selectedTask}
        priorities={priorities} // 👈 Yeni sütun
        statuses={statuses}     // 👈 Yeni sütun
      />
      ...
      <TaskDetailModal show={showDetailModal} handleClose={() => {setShowDetailModal(false); setDetailTask(null);}} task={detailTask} />
    </div>
  );
}
